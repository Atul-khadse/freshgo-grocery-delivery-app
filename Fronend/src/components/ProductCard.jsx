  ...
  <div className="text-gray-500/60 text-sm mt-2">
    ...
    <div className="flex items-center gap-0.5">
      {Array(5)
        .fill('')
        .map((_, i) => (
          <img key={i} className="md:w-3.5 w-3" src={rating_star} />
        ))}
      <p>({5})</p>
    </div>
    <div className="flex items-end justify-between mt-3">
      ...
      <div onClick={(e) => e.stopPropagation()} className="text-primary">
        {!cartItems[product._id] ? (
          <button
            className="flex cursor-pointer items-center justify-center gap-1 bg-primary/10 border border-primary/40 md:w-[80px] w-[64px] h-[34px] rounded text-primary font-medium"
